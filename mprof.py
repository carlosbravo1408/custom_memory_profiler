        ...
        for arg in args:
            ...
            if index is not None:
                try:
                    ...
                    raise ValueError(
                        "Invalid index (non-existing file): %s" % arg)
...
def run_action():
    ...
    attach_arg = parser.add_argument("--attach", "-a", dest="attach_existing",
                                     action="store_true",
                                     help="Attach to an existing process, by process name or by pid")
    parser.add_argument("--timeout", "-t", dest="timeout", action="store",
                        type=int,
    ...
    parser.add_argument("--output", "-o", dest="filename",
                        default="mprofile_%s.dat" % time.strftime(
...
This file contains the process memory consumption, in Mb (one value per line).""")
    parser.add_argument("--backend", dest="backend",
                        choices=["psutil", "psutil_pss", "psutil_uss", "posix",
                                 "tracemalloc"],
                        default="psutil",
    ...
    if args.attach_existing:
        print(
            'attaching to existing process, using hint: {}'.format(program[0]))
        ...
        if args.python:
            ...
            if not program[0].startswith("python"):
                program.insert(0, sys.executable)
            cmd_line = get_cmd_line(program)
            extra_args = ["-m", "memory_profiler", "--timestamp", "-o",
                          mprofile_output]
    ...
    with open(mprofile_output, "a") as f:
        f.write("CMDLINE {0}\n".format(cmd_line))
        mp.memory_usage(proc=p, interval=args.interval, timeout=args.timeout,
                        timestamps=True,
...
def read_mprofile_file(filename):
    ...
    timestamp_mem = []
    system_cpu_usage = []
    timestamp_system_cpu = []
    process_cpu_usage = []
    timestamp_process_cpu = []
    process_cpu_core = defaultdict(list)
    ...
    for l in f:
        ...
        if field == "MEM":
            ...
            timestamp_mem.append(float(values[1]))
        elif field == ('CPU_S'):
            values = value.split(' ')
            system_cpu_usage.append(float(values[0]))
            timestamp_system_cpu.append(float(values[1]))
        elif field == ('CPU_P'):
            values = value.split(' ')
            process_cpu_usage.append(float(values[0]))
            timestamp_process_cpu.append(float(values[1]))
        elif field == "FUNC":
            values = value.split(' ')
            ...
            values = value.split(' ')
            corenum = values[0]
            process_cpu_core[corenum].append(
    ...
    return {
        ...
        "process_cpu_core": process_cpu_core,
        "timestamp_mem": timestamp_mem,
        "timestamp_system_cpu": timestamp_system_cpu,
        "timestamp_process_cpu": timestamp_process_cpu,
...
def plot_file(filename, index=0, timestamps=True, children=True, options=None):
    ...
    if len(mprofile['timestamp_mem']) == 0:
    ...
    t = mprofile['timestamp_mem']
    t_system_cpu = mprofile['timestamp_system_cpu']
    t_process_cpu = mprofile['timestamp_process_cpu']
    mem = mprofile['mem_usage']
    chld = mprofile['children']
    cpu_system = mprofile['system_cpu_usage']
    cpu_process = mprofile['process_cpu_usage']
    cpu_core = mprofile['process_cpu_core']
    ...
    def _sort(timestamp_array, profiler_array):
        ind = timestamp_array.argsort()
        profiler_array = profiler_array[ind]
        timestamp_array = timestamp_array[ind]

    def get_max(profile_array):
        return profile_array.max(), profile_array.argmax()

    mem = np.asarray(mem)
    t = np.asarray(t)
    t_system_cpu = np.asarray(t_system_cpu)
    t_process_cpu = np.asarray(t_process_cpu)
    cpu_system = np.asarray(cpu_system)
    cpu_process = np.asarray(cpu_process)
    _sort(t, mem)
    _sort(t_system_cpu, cpu_system)
    _sort(t_process_cpu, cpu_process)

    # Plot curves
    global_start = min(float(t[0]), float(t_system_cpu[0]), float(t_process_cpu[0]))
    t = t - global_start
    t_system_cpu = t_system_cpu - global_start
    t_process_cpu = t_process_cpu - global_start

    all_colors = ("c", "y", "g", "r", "b")
    line_colors = ("k", "b", "r", "g", "c", "y", "m")

    show_trend_slope = options is not None and hasattr(options,
                                                       'slope') and options.slope is True
    def draw(axis, profile, timestamp, _children, line_label):
        trend = None
        if show_trend_slope:
            # Compute trend line
            trend = np.polyfit(timestamp, profile, 1)

            # Append slope to label
            line_label = line_label + " slope {0:.5f}".format(
                trend[0])
        axis.plot(timestamp, profile, "+-" + line_colors[index % len(line_colors)],
                 label=line_label)

        if show_trend_slope:
            # Plot the trend line
            axis.plot(timestamp, timestamp * trend[0] + trend[1], "--", linewidth=0.5,
                     color="#00e3d8")
        ...
        if len(_children) > 0 and children:
            cmpoint = (0, 0)  # maximal child memory

            for idx, (proc, data) in enumerate(_children.items()):
                ...
                child_mem_trend_label = " slope {0:.5f}".format(
                    cmem_trend[0])
                # Plot the line to the figure
                axis.plot(cts, cmem,
                         "+-" + line_colors[
                             (idx + index + 1) % len(line_colors)],
                         label="child {}{}".format(proc, child_mem_trend_label))

                if show_trend_slope:
                    # Plot the trend line
                    axis.plot(cts, cts * cmem_trend[0] + cmem_trend[1], "--",
            ...
            axis.vlines(cmpoint[0], pl.ylim()[0] + 0.001, pl.ylim()[1] -
                       0.001, 'r',
                       '--')
            axis.hlines(cmpoint[1], pl.xlim()[0] + 0.001, pl.xlim()[1] -
                       0.001, 'r',
                       '--')
        ...
        if len(ts) > 0 and timestamps:
            ...
            for f, exec_ts in ts.items():
                for execution in exec_ts:
                    add_brackets(execution[:2], execution[2:],
                                 xshift=global_start,
        ...
        if timestamps:
            max_vaule, max_index = get_max(profile)
            axis.hlines(max_vaule,
                       pl.xlim()[0] + 0.001, pl.xlim()[1] - 0.001,
                       colors="r", linestyles="--")
            axis.vlines(timestamp[max_index], bottom, top,
                       colors="r", linestyles="--")

    line_label = time.strftime("%d / %m / %Y - start at %H:%M:%S",
                               time.localtime(global_start)) \
                 + ".{0:03d}".format(
        int(round(math.modf(global_start)[0] * 1000)))
    draw(ax1, mem, t, chld, line_label)
    draw(ax2, cpu_process, t_process_cpu, {}, 'Script CPU usage')
    index = index + 1
    draw(ax2, cpu_system, t_system_cpu, cpu_core, 'System CPU usage')
    return mprofile
...
def function_labels(dotted_function_names):
    ...
    fn_to_label = dict(
        (label_state["functions"][0], label) for label, label_state in
        state.items())
...
such file in the current directory."""
    ...
    ax1.set_ylabel("memory used (in MiB)")
    ax2.set_ylabel("CPU used (%)")
    if args.title is None and len(filenames) == 1:
        fig.suptitle(mprofile['cmd_line'])
...
using `mprof run`. If no .dat file is given, it will take the most recent
such file in the current directory."""
    parser = ArgumentParser(usage="mprof peak [options] [file.dat]",
                            description=desc)
    parser.add_argument("profiles", nargs="*",
                        help="profiles made by mprof run")
